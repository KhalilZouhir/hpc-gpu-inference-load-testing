...
export const options = {
  scenarios: {
    production_load: {
	  ...
	  // there is other options like ramping arival rate , 'constant-vus' check documentation to know with executor to go with it all depends on 
	  // on ur objectives 
      startVUs: 1,
      stages: [
		  ...
		  { duration: '2m',  target: 50 },  // brief hold
  ...
  thresholds: {
    http_req_duration:  ['p(95)<40000'],     // 95% < 40s
    http_req_failed:    ['rate<0.02'],       // <2% errors
    tokens_per_second:   ['p(50)>28','p(10)>20'], // Throughput 
    total_tokens:       ['p(95)>100'],       // to check if there any prompts with no responses 
    errors:             ['rate<0.02'],
    time_to_first_token: ['p(95)<2000'] // TTFT is inferior then 2 seconds (2000 ms)
  },
  // systemTags let you filter in Grafana (status, method, url, etc.)
...
// up to u to change this timeout duration 
  // Slightly longer timeout for long prompts
  const timeout = '240s';  // use one timeout for all categories during load

//update the path 
  const payload = JSON.stringify({
    model: "/home/skiredj.abderrahman/LLM1-Test/gemma-3-27b-it-int4-awq",
    prompt: prompt.text,
  ...
  if (ok && res.status === 200) {
    try {
      ...
      // Non-streaming approximation: first token often ~5–20% of total latency so we set 12% but its not accurate 
